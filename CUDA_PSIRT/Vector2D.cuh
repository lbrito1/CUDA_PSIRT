...
__device__ void set_vector(Vector2D* a, double x, double y)
{
	a->x=x;a->y=y;
}
...
__device__ void minus_void(Vector2D* a, Vector2D* b, Vector2D* result)
{
	result->x = a->x-b->x;
	result->y = a->y-b->y;
}
...
Vector2D* new_vector(double x, double y) {
	Vector2D* a;
	cudaMalloc((void**)&a, sizeof(Vector2D));	//TODO 
	a->x=x;
	a->y=y;
	return a;
}
...
{
	Vector2D* cloned;
	cudaMalloc((void**)&cloned,sizeof(Vector2D));
	cloned->x = a->x;
	cloned->y = a->y;
	return cloned;
}
...
__device__ void RotateCounterClockWise(Vector2D* a, float angle, Vector2D* rotated)
{
...
__device__ void RotateClockWise(Vector2D* a, float angle, Vector2D* rotated)
{
	RotateCounterClockWise(a, -angle, rotated);
}


__device__ double vector_vector_distance(Vector2D* a, Vector2D* b)
{
  Vector2D c; 
  minus_void(a,b,&c);
  return magnitude(&c);
